...
class Config:
    ...
    def convert_extension(self) -> list:
        """変換対象拡張子の取得

        Returns:
            list: 変換対象拡張子のリスト
        """
        temp_path = self._config_data.get("convert_extension", "")
        if not temp_path:
            # デフォルト拡張子リスト
            temp_path = ['.c', '.cpp', '.h', '.hpp']
        return temp_path
    ...
    def backup_extension(self) -> str:
        ...
        Returns:
            str: バックアップ拡張子
        """
        temp_path = self._config_data.get("backup_extension", "")
        if not temp_path:
            # 空文字を返して、バックアップ拡張子が設定されていない場合の処理を呼び出し元に任せる
            temp_path = ""
        return temp_path
    ...
    def _load_settings(self) -> Dict[str, Any]:
        ...
        if not os.path.exists(self._settings_file):
            ...
            temp_dict = {
                "convert_extension": ['.c', '.cpp', '.h', '.hpp'],
                "backup_extension": ".org",
                "output_path": './',
            }
        else:
            # settings.yamlファイルの読み込み
